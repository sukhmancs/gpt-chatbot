...
class MathBot:
    ...
    def __init__(self, api_key):
        ...
        self.gpt_turbo_dialog = self.generate_dialog() # generate a dialog for the Davinci-002 model
    ...
    def gpt_turbo(self, utterance):
        ...
        """
        # append the user's message to the dialog
        self.gpt_turbo_dialog.append({"role": "user", "content": utterance})
        
        # generate the response
        response = self.client.chat.completions.create(model="gpt-3.5-turbo-0125", messages=self.gpt_turbo_dialog, temperature=0, max_tokens=1)

        # append the response to the dialog so that it can be used in the next iteration
        # to generate the next response. For example, if the user asks a follow-up question.
        # the dialog will contain the context of the conversation.
        self.gpt_turbo_dialog.append({"role": "assistant", "content": response.choices[0].message.content})
        return response.choices[0].message.content # return the response
    ...
    def get_response(self, utterance, model="both"):
        ...
        if model == "gpt-3.5-turbo":
            ...
            return self.gpt_turbo(utterance)
        elif model == "both":
            response1 = self.gpt_bot(utterance)
            response2 = self.gpt_turbo(utterance)
    ...
    def reset_dialog(self):
        ...
        self.gpt_turbo_dialog = self.generate_dialog() # reset and generate a new dialog for the Davinci-002 model
...
if isinstance(response, tuple):
    print("Both models were used to generate the response.")
    print(f"Turbo: {response[0]}\nDavinci: {response[1]}")
else: # response is a string
    print("Only one model was used to generate the response.")
    print(response)
